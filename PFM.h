//
//  PFM.h
//  copied from: https://stackoverflow.com/questions/29487925/opencv-how-to-read-pfm-files
//
...
class PFM {
public:
    PFM() { printf("Constructor is working! \n"); }
    
    bool inline is_little_big_endianness_swap() {
    ...
    template<typename T> inline T * read_pfm(const std::string & filename) {
        FILE * pFile;
    ...
    template<typename T> inline void write_pfm(const std::string & filename, const T* imgbuffer, const float & endianess_) {
